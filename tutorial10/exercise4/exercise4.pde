...
int buildCar(int idx, float ox, float oz, float s, color chassisCol, color cabinCol) {
  ...
  chassis.tag = "car" + ((idx) / partsPerCar);
  parts[idx++] = chassis;
...
void mousePressed() {
  ...
  if (newSelection == selectedCar) {
    selectedCar = -1;     // click same car again -> deselect
  } else {
    selectedCar = newSelection;
  }
  
  // Begin a drag only if we landed on a car
  dragging = (selectedCar >= 0);
